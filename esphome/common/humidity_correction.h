...
inline float f_to_c(float tempF)
{
    return ((tempF - 32) / 1.8);
}

inline float c_to_f(float tempC)
{
    return (tempC * 1.8 + 32);
}

inline float c_to_k(float tempC)
{
    return tempC + 273.15;
}

inline float f_to_k(float tempF)
{
    return f_to_c(tempF) + 273.15;
}

inline float c_to_r(float tempC)
{
    return (c_to_k(tempC) * 1.8);
}

inline float psia_to_mPa(float psia)
{
    return psia * 68.9476;
}

inline float psia_to_pa(float psia)
{
    return psia * 6894.76;
}

inline float compute_dew_point(float tempC, float humidity)
{
...
inline float dew_point_to_RH(float tempC, float dpC)
{
  const float a = 17.625;
  const float b = 243.04;

  float rh = 100.0 * (exp(a * dpC / (b + dpC)) / exp(a * tempC / (b + tempC)));
  return rh;
}

inline float sat_pressure(float tempC)
{
...
inline float correct_humidity(float temperature1, float humidity1, float temperature2)
{
